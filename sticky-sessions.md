...
peut être comme ceci:
 ...
 - Cette demande est servie par le load Balancer, qui la transmet à un nœud keycloak aléatoire (par exemple, node1). 
   ...
   (adresse IP du client, etc.). Tout dépend de la configuration du Load Balancersous-jacent (proxy inverse).
...
À partir de ce moment, il est avantageux que l'équilibreur de charge transmette toutes les demandes 
...
de keycloak pour les raisons mentionnées ci-dessus. Vous devez configurer votre équilibreur de charge pour qu'il 
colle au cookie AUTH_SESSION_ID. 
